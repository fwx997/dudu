...
- [x] D1 以 `香色闺阁_2.56.1.ipa` 的 `plist_localConfig`、阅读设置 plist 和图片资源为布局基线
- [x] D2 提取四色书架文件图标、阅读工具图标和纸张纹理到 `Resources/Assets.xcassets`
- [x] D3 将原版排版默认值同步到阅读器（20 号字、行距 10、段距 14、左右 20、上 50、下 30）
- [x] D4 记录 IPA 哈希、版本和资源映射到 `docs/ipa-reference/manifest.json`

## E. 1:1 收敛实施计划

### E1 证据登记（已完成）

- [x] E1.1 记录 IPA 解包资源、plist、版本和哈希
- [x] E1.2 交叉核对公开 IPA、书源仓库和内容类型资料，记录在 `docs/online-research.md`
- [x] E1.3 以 `plist_settingKeyInfo` 重新核对阅读器默认值

### E2 统一视觉系统（进行中）

- [x] E2.1 书架四色文件图标和阅读器图标使用 IPA 原图
- [x] E2.2 阅读器纸张纹理、米黄背景和默认排版参数接入
- [ ] E2.3 把首页、站点页、目录页、设置页的字号/行高/边距收敛到同一组设计 token
- [ ] E2.4 用原版资源替换剩余系统 SF Symbol 和通用 SwiftUI 控件

### E3 逐屏复刻（进行中）

- [x] E3.1 首页顶部结构、书架列表、发现站点列表
- [x] E3.2 站点管理导入/编辑/检测/多选
- [x] E3.3 源内浏览分类、筛选、搜索和切换站点
- [x] E3.4 阅读器顶部菜单、底部面板、目录/书签
- [ ] E3.5 书籍详情、搜索结果、设置页逐屏截图校准

### E4 交互状态复刻

- [x] E4.1 `.xbs` 文件关联、打开和导入
- [x] E4.2 书架长按编辑、移动、删除、手动排序
- [ ] E4.3 补齐加载中、空数据、网络失败、禁用站点和缓存状态的原版文案/布局
- [ ] E4.4 补齐状态栏、侧滑返回、自动隐藏小横条等阅读设置行为

### E5 验收

- [x] E5.1 CI 编译和未签名 IPA 产出
- [ ] E5.2 同一设备尺寸逐屏截图差异对比
- [ ] E5.3 真机完整流程清单
- [ ] E5.4 通过后再标记 C3/C4 完成
